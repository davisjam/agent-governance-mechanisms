...
| | |
|---|---|
| Summary | Match a control's enforcement scope to the semantic scope of the property it checks. |
| Target | Agent · **Governance-doc controls** |
| Form | `agent-output` |
| Enforcement | **Soft** — a design-time placement judgment made when a control is built or reviewed; it aims where a control lands, it does not itself block |

## Motivation — the failure it kills

A property has a **scope at which it becomes legible** — the smallest window in which enough of the world is
visible to decide it true or false. Place a control below that scope and it fails one of two ways:
...
The reflex is to place a control where it is cheapest and earliest — the pre-commit hook, the single
...
control's design, and it is silent: a mis-placed control looks wired, runs green (or red) on every fire, and
...
- **Not just "run the check later."** Later is a *time* axis; this is a *scope* axis. A check can run late
  ...
  session-start hook that reads the whole alert backlog). The move is to match the control's window to the
  property's window, whenever in time that window opens.

## Mechanism

When you build or review a control, name the property, then find the scope at which it is legible, then place
the control there.
...
- **Place the control at that scope, and no lower.** Bind it to the lifecycle event or gate whose window
...
than the last. A control belongs on the lowest rung at which its property is decidable.

## Prerequisites

- **A ladder of enforcement points at distinct scopes.** Without a runtime that exposes a commit event, a
  task-return event, and a unit-close gate, there is nowhere to place a control but the one hook you have.
...
- **A reviewer who asks the placement question at design time.** This is a soft judgment; it holds only if
  someone reaching for a new control asks *at what scope is this legible?* before defaulting to the cheapest
  hook. A failure-interpretation handoff that routes a recurrence to *design a control* is the natural place
...
- **It is a judgment, not a gate.** Nothing forces a designer to place a control correctly; a mis-placed
  control still ships and still looks wired. The defense is the design-time question, and a soft question can
  be skipped. The tell of a skipped one is a control that fires forever and never catches its class.
...
- **A banned API → one changed file.** The counter-example that keeps the principle honest: a property fully
  ...
  ladder; most controls belong at the bottom.
...
- **See also (a placement each)** — [pre-commit-hook](../gates-and-merge-train/pre-commit-hook.md) sits at the
  ...
  runtime-event rung. Each is a control *placed* by this judgment; this entry is the judgment that places
  them. The named axis is *placement-principle* vs *placed-control*.
- **Consumer** — the operate→harden handoff ([operator-runbook-skill](operator-runbook-skill.md)) routes a
  recurring failure to *design a control*, and part of that design is asking this placement question before
