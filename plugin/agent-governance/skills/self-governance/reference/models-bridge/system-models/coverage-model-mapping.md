...
| Form | `validation` |
| Enforcement | **Soft·Hard** — a per-node coverage map that *surfaces* untested nodes (a backlog); promotable to a gate that *requires* a covering test for load-bearing nodes |

## Motivation — the failure it kills

Line and branch coverage tell you what *fraction of the code* ran under test — not *which of the system's
invariants* are actually exercised. A model can show 90% line coverage while a load-bearing race invariant
...
**work-list** ("cover *this* invariant"), not a percentage to chase. The distinction is *coverage of
lines, aggregated* versus *coverage of the model's meanings, per node*.
...
by-which-tests. Two uses ride on it — a **backlog** (the uncovered load-bearing nodes are the next tests
to write; a sweep walks them), and, promoted, a **gate** (a load-bearing invariant node with no covering
...
- **A "load-bearing" policy** — which nodes *must* be covered (the gate's scope), so the map drives a
...
- **Backlog-vs-gate is a judgment.** Gating every node starves throughput; gating none leaves criticals
  untested — the load-bearing scope is the tuning surface.
...
- The model-as-work-list: a sweep that walks the uncovered load-bearing nodes and writes the missing tests.
- (Promotable) a gate requiring a covering test for each load-bearing invariant node.
...
- **Counterpart** — [formal-invariant-verification](formal-invariant-verification.md): that *proves* an
  ...
  Proof vs exercise — complementary, and a load-bearing node ideally has both.
