...
> means a fix or a constraint applied once holds everywhere (→ A.19). Enforced by a sole-seam ban-lint
> on the raw alternative. See the *canonical models & seams* control in the catalogue.

Note the moves: an arrow to the Part A principle it instantiates (`→ A.19`), a named enforcement
...
prevents the class from recurring (Part A.8 + A.21: *convert a recurring failure into a control*). Common
sections a mature Part B also carries, all project-specific: conversation conventions, CLI commands,
deploy discipline, architecture overview, library / dependency discipline, testing tiers, and the
agent-dispatch / orchestration machinery. Grow it deliberately — the "what earns a spot" test above is
what keeps it from bloating into noise every agent has to page through.
