...
**Alignment Thesis.** *Encode obligations into the environment.* A governance mechanism the environment enforces holds implementation to intent. A policy decided once then stands against every later change, so confidently-wrong work is made visible instead of shipped.
...
**Constraint.** *Prevent the mistake.* A governance mechanism that prevents drift by scoping the action space, so the mistake is impossible to make in the first place.

**Sensor.** *Detect the mistake.* A governance mechanism that catches a violation after the fact instead of preventing it, failing the iteration so the mistake cannot ship unnoticed.
