...
```mermaid
flowchart TB
    subgraph INTER["Interaction level: the conflict is legible only here"]
        R(["shared resource:<br/>the one commit-set"])
        X{{"contradiction:<br/>two incompatible demands on it"}}
        R --- X
    end
    subgraph CODE["Code level: each control is correct read alone"]
        A["Control A<br/>requires a squashed commit ✓"]
        B["Control B<br/>flags out-of-brief changes ✓"]
    end
    A -->|acts on| R
    B -->|acts on| R
    accTitle: The semantic gap — a conflict invisible in either control's code
    accDescr: Two boxes on a lower code level, Control A which requires a squashed commit and Control B which flags out-of-brief changes, are each locally correct when read alone. Both send arrows up to a shared resource, the one commit-set, on an upper interaction level, where a contradiction marker sits on the resource: two incompatible demands on the one thing. The defect is in neither control's code below; it appears only when both are drawn against the shared resource above. That vertical distance is the semantic gap.
```

*The conflict is in neither control's code — read either alone and it is correct. It becomes legible one
level up, where both controls are drawn against the shared resource. That distance between the code and
the interaction is the semantic gap the graph closes.*
...
draws exactly those crashes — the static catalogue's dynamic dual. A catalogue answers *what each
control is*, a list of nodes with no edges; the graph answers *which pairs can collide, and over
what* — the edges the list omits. And the two halves of that answer split cleanly. Some edges are
