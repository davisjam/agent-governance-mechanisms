...
John Ousterhout gives the rule in one line [cite: ousterhout2018]: **measure one level deeper than the number you
...
<!-- figure: assets/metric-spectrum.svg | Metrics run hard to soft. The harder the metric, the more a machine can block on it alone; the softer, the more it can only aim a reader who must still judge. The flagship sits in the middle: entry-point-native coverage, the deeper cut that names the untested invariant. -->
...
Turn the arrow around and a second metric appears. Walk from each *test* to the code it
...
<!-- figure: assets/mmm-drain-curve.svg | The Missing-Model-Metric drain. The orphan rate — tests whose exercised code reaches no model-anchored symbol — falls from 56% to 14.9% across eight tracer re-runs, each following a model-loop Epic that modeled the biggest remaining orphan cluster. Code-to-model coverage rises as the mirror image, from 44% toward 85%. The dashed line marks the loop's ≤10% target. -->
...
remaining orphan cluster; an Epic then builds the missing typed model and its invariants
...
orphan rate from 56% at the pilot to 14.9% at the latest committed point — code-to-model
coverage rising from 44% to about 85%. The metric drove the modeling, and the falling curve
is the record of the modeling landing.

Two honesty notes bound the claim. The population is comparable across runs but not
fixed: the denominator drifts from 144 to 148 tests as the suite evolves, and the first
