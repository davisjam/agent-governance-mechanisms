...
<!-- figure: assets/mmm-drain-curve.svg | The Missing-Model-Metric drain. The orphan rate — tests whose exercised code reaches no model-anchored symbol — falls from 56% to **7.89%** across **nine** tracer re-runs, each following a model-loop Epic that modeled the biggest remaining orphan cluster. Code-to-model coverage rises as the mirror image, from 44% toward **92%**. The dashed line marks the loop's ≤10% target — **now crossed** (and 0% once glue-code residue is excluded). -->
