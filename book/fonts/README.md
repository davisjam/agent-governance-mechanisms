...
The web book loads Source Serif 4 / Source Sans 3 / IBM Plex Mono from Google Fonts (see
...
## Family-name reconciliation (matching by `name` table, not filename)

Typst matches fonts by the name baked into each file's `name` table, not by filename — so a family whose
static instances split across per-optical-size family names must be reconciled with the `typst-stack`
token. Source Serif 4 needs no such reconciliation (see below); an earlier display face, Fraunces, did
(its instances reported `"Fraunces 9pt"`, not `"Fraunces"`) — that face has since been retired.
