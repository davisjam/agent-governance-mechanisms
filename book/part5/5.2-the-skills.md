...
<!-- noqa: book-visual — a tour of three shipped skills, described by what each triggers on and does; the one structural shape (base-model + orthogonal-models + SKILL.md construction) is drawn out in Appendix E, which this chapter forward-points to, and the self-operate lifecycle loop is already figured in the loops chapter. No new shape here that a figure would carry better than the prose. -->
