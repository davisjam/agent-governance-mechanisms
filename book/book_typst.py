"""IR → Typst emitter — a SPIKE sibling to the HTML renderer.
...
HTML and Typst PDF become two emitters over one IR, neither derived from the other.

WHY (the decision this de-risks).  Today's PDF is HTML → Paged.js (headless browser) → PDF, which inherits
browser artifacts, bloat, imperfect print typography, and a headless-browser dependency. `IR → Typst → PDF`
is the print-native alternative recorded in `book/IR-DESIGN.md` §"PDF generation — output via Typst". This
emitter answers two questions with evidence: (1) is the Typst PDF good (fidelity + small + fast)? and (2)
can Typst carry ALL our model annotations (the feasibility study for a possible future source migration)?

DISCIPLINE — SPIKE = OUTPUT ONLY.  This module is stdlib (it emits text) and READS `build_book_html` /
`book_ir` for the tokenizer, the IR, mermaid-SVG rendering, figure-asset resolution, and metrics — it does
NOT modify either. The production HTML + Paged.js path is left fully intact.
...
_PREAMBLE = """\
// GENERATED by book/book_typst.py — the IR→Typst SPIKE emitter. Do not hand-edit; regenerate via
//   python3 book/book_typst.py <chapter-slug> [<slug> …]
// This is a spike artifact (gitignored). The production PDF path is HTML → Paged.js, untouched.
#set document(title: "MAGE — Typst spike")
...
def emit_document(slugs: list[str], root: pathlib.Path | None = None) -> str:
    """Emit a standalone Typst document for the named chapter slugs (one `#pagebreak()`-separated section
    each). `root` is the Typst compilation root the image paths resolve against (defaults to the repo dir,
    the parent of book/)."""
    ...
    for n, slug in enumerate(slugs):
        ...
        parts.append(render_chapter(by_slug[slug], ctx))
    return "\n\n".join(parts) + "\n"
