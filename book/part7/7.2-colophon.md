...
## The production, briefly

This book was set in {{dt:font_display}} for display, {{dt:font_body}} for the body, and {{dt:font_mono}} for code, on a warm paper stock with a single {{dt:accent_name}} accent. There was no separate typesetting pass. The manuscript is one structured source — chapters, sections, figures, and citations — that a single build script projects two ways [gh:book/build_book_html.py]: to HTML for the web edition, and to Typst for the print PDF, which `typst` then lays out. The web book and the print book are two views of one source, so they cannot drift apart. The whole thing is kept under version control, validated and built by one stdlib-only tool that runs on a fresh checkout [gh:catalog.py]. References live in a BibTeX file [gh:book/references.bib], resolved through inline markers, with a gate that refuses to build on a stale citation. The figures are hand-drawn SVGs — no plotting library behind them — each carrying a title and a description for readers arriving by screen reader.
