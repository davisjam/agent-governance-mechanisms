...
substrate change before making it; and hold the governing document itself as enforced, queryable
infrastructure rather than prose that rots. Five parts make the estate legible to itself, questionable for
its own blind spots and change-safety, and deliverable to every agent.

<!-- label: governance-of-governance-stack -->
<!-- figure: assets/governance-of-governance-stack.svg | The governance-of-governance stack in one picture. Five parts run left to right. Model the estate (violet): GRAPH represents each control as a node tagged by trigger and resource footprint, with a conflict edge where two controls contend; REGISTRY attaches machine-readable metadata to each governance rule. Query the estate (green): CENSUS classifies each control by the target it guards, so a bare target is a re-derived gap; RADIUS makes each control declare its substrate assumption, so a change's blast radius is a computed query. Deliver it (accent): INDEX holds the governance document as a numbered, enforced, capped rule index in every agent's boot context. -->
...
"what will my change break." **REGISTRY** makes the governing prose itself a queryable model. **INDEX** is
...
### 4. REGISTRY — the governance prose as a queryable model
...
### 5. INDEX — the governing document as enforced infrastructure
