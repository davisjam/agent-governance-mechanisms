...
<!-- table: Model-sync evidence — the derived floor under load, model↔code sync only, with documentation drift excluded by construction. The top rows are the floor holding: heavy model-bridge churn, zero drift reaching a post-close reopen, and a handful of genuine drifts the derived lint caught at HEAD. The pre-floor row is the class it was built to close — real drift that escaped past green definitions-of-done before the floor existed. [short: Model-sync evidence — the derived floor under load] -->
...
it holding. [data: model-sync-efficacy]
...
<!-- figure: assets/model-sync-two-layer-net.svg | *The Two-Layer Net.* The model↔code gap spanned by two layers, with two escape hatches drawn as gaps in the net. Derived defends, snapshotted drifts — and what isn't modeled can't drift-check. -->
