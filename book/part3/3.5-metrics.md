...
A metric senses. A playbook acts. A control enforces. Those three make up a governed loop, and
the metric comes first because the other two are blind without it. A playbook that fires on a
number that measures the wrong thing does the wrong work. A control that grades on a number too
...
the container boots, the runtime loads its classes, and the first real work runs. That wait is
the cold-start latency, and it is as hard as a metric gets: a stopwatch on a probe, the same
number on every run. The system measures it per service — `render-service` at 850 ms cold
against 319 ms warm, while a service that only proxies an external API carries no cold tax to
speak of.

Per-service is already one level deeper than a fleet average, and it drives the first decision:
which service warrants a lever — a CPU boost, a slimmer image, a warm floor. But the sharper
question is not any single service. It is the *chain*. A request that lands when every
...
The sharpest instance ties metrics back to controls, and it is worth ending on because it is
...
The grade is a metric — a distance, read from the model at check time — and it drives a control's
