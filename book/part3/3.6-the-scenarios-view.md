...
and the scenario is the walk. Because the walk re-checks the other four against a
real goal, the scenario is where the models are held honest — the Alignment
Thesis turned back on the views themselves.
...
<!-- figure: assets/join-composite.svg | *The Four-Model Join.* Four models compose on one shared key so a product goal reaches all the way to how the deploy runs its tests, with nothing said twice. The user-journey model names a journey and its endpoints; the coverage-to-node map asks whether those endpoints are tested; the criticality-to-placement Selector derives which host tier each test runs on from the journey's criticality; and the Scheduler rations the resulting set per host. Read it left to right — a journey's criticality derives its test tier, coverage joins on the same endpoint node, and the Scheduler runs the set in parallel when only a cost gate applies, serialized when a scarce host is in play. -->
...
Two pieces of real policy code make the join concrete. The **Selector** derives a
...
We do it because an agent will pay the upkeep, and because there is no cheaper way to trust what a
fleet ships.
