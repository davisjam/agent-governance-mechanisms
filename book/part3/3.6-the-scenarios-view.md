...
returned 200, a card appeared — and can green while the task the user came for is broken: the artifact
...
post-condition: a small boolean expression — AND, OR, NOT — over reusable observable predicates, *"the
artifact rendered AND a control is inspectable AND no error-fallback is shown."* The leaves come from a
...
assertion in the fast local tier, and a headless probe against the pre-promotion canary — which
