...
<!-- point: a-user-journey-anchors-the-view-with-five-real-models | A user journey anchors the view; five models embody it. | terms: user-journey-model, agent-orchestration-model, journey-criticality-test-placement, coverage-model-mapping, journey-task-closure -->
...
journey-criticality-to-test-placement, and coverage-to-model-node mapping. A fifth, journey
task-closure, sits alongside them: it types what the journey's terminal assertion must *mean*,
a concern the join's four do not touch. The chapter closes with the zoo's flagship demonstration —
four models joined so a product goal reaches all the way to how the deploy rations its tests.

Two insets a reader needs for this view's model pages:

<!-- point: node-coverage-counts-meanings-where-line-coverage-counts-lines | Node coverage counts meanings where line coverage counts lines. | terms: node-coverage, line-coverage -->
...
<!-- point: task-closure-types-what-the-terminal-assertion-must-mean | Task-closure types what the terminal assertion must mean. | terms: journey-task-closure, closure-post-condition, flow-vs-task -->
...
asks what the test asserts at the end. A journey test usually asserts that the flow ran: a page
returned 200, a card appeared. It can green while the task the user came for is broken, the artifact
...
<!-- point: a-flow-only-closure-on-a-major-journey-is-a-finding | A flow-only closure on a major journey is a finding. | terms: closure-strength, flow-vs-task, closure-meaning -->
...
That asymmetry is the model's teeth. This model assesses **closure meaning**, the axis its three
...
Watch one fact — a journey is *major* — travel through four models with nothing said twice.
The other model pages each show a single model; this one shows them **joining, not repeating.**
The user-journey model (M13) names the journey and its
...
Scheduler's profile decides whether that host fans those tests out or serializes them (M6→M7).
...
per host, in [ref:join-composite]:
...
The join's invariants span all four models, collected in [ref:scenarios-invariants]; two are the trunk derive-the-checker discipline.

<!-- label: scenarios-invariants -->
<!-- table: Invariants of the Scenarios view — the properties the journey-to-test join spans, each with its temporal shape and check. [short: Invariants of the Scenarios view] -->
...
the models honest, trace every requirement to the code that realizes it — the rest of us now do too. We
do it because an agent will pay the upkeep, and because it is the cheapest way to trust what the fleet ships.
