...
<!-- figure: assets/join-composite.svg | Four models, one join: a journey's criticality derives its test tier, coverage joins on the same endpoint node, and the Scheduler rations the resulting set per host — parallel when only a cost gate applies, serialized when a scarce host is in play. -->
