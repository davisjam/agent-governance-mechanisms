...
<!-- index-def: controls-vs-architecture -->
...
<!-- figure: assets/control-vs-architecture.svg | The two forms a failure-to-mechanism conversion can take. A recurring failure, seen twice, becomes a mechanism — either a control or an architecture. A control is a sensor: it lets the mistake happen but detects it in time to stop the damage (a test suite in CI, a smoke detector). An architecture is a wall: it makes the whole class of mistake impossible by construction (a typed enumeration over free-form strings, a firewall). The control watches for the bug; the wall forbids it. -->

<!-- index-def: control-sensor -->
...
wrong. It is the wiring under every control: the event a gate fires on, the log a checker
reads, the trace that turns "the build failed" into "the build failed *here, for this
reason.*" Build the observability first and the control has something to watch; skip it
and the sensor stares at a blank wall.

<!-- index-def: architecture-wall -->
