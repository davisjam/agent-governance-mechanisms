<!-- STATUS: DESIGN-STAGE PROVISIONAL — held in _drafts/ (outside the role trees), so neither the -->
<!-- entry validator nor the render/serve path sees it. NOT wired into the census, INDEX.md, or -->
<!-- catalogue-figure; the shipped count stays 65. -->
<!-- The mechanism it describes is a ratified design (user-approved 260722); nothing is built or -->
<!-- instantiated yet — no edge-intent enum (CORRECTNESS/COST_GATE/LOAD), no typed Scheduler, no -->
<!-- per-host (concurrency-ceiling, budget) profile, no lints — on main or in any worktree. -->
<!-- Catalogue bar #3 — "its examples instantiate it" — is not met until the build lands. -->
<!-- To promote once the artifacts ship: move to models-bridge/system-models/invariant-dag-execution-policy.md, -->
<!-- rewrite Known-uses from "designed, not yet landed" to real artifacts named once, add the INDEX.md row, -->
<!-- bump the subtotals + grand total (65 -> 66) and census tokens, add a catalogue-figure.html chip, -->
<!-- then `python3 catalog.py validate` -> 0. -->
...
intent, with `LOAD` edges migrated out to a `Scheduler` that reads a per-host `(concurrency-ceiling,
budget)` profile).

| | |
|---|---|
| Summary | A deploy DAG carries correctness + cost-gate edges only; a typed Scheduler reads a per-host profile and rations load + budget over it. |
...
- **A typed Scheduler over a per-host profile.** A `Scheduler` maps a host profile
  ...
  environment is a **new profile row**, a `(ceiling, budget)` tuple — never new graph code. The Scheduler
  is state-bearing (it holds live permit counts and the per-host profile), so it is a class, and it emits
  its rationing decisions as a timestamped artifact — for each wave, how many nodes were ready, how many
  launched, how many queued and for how long — so the plan *acts on* load rather than only enforcing it.

- **Two lints holding the separation.** A stable lint reads the built graphs and asserts the invariant: no
  edge carries the `LOAD` intent (load rationing belongs to the Scheduler), and the edge set is identical
  across hosts over their roster intersection (a static check that any surviving `env` branch in the graph
  builder adds or removes *steps*, never *edges*). A sibling file-name lint forbids re-introducing the stale
  driver name the split retires. Both land audit-only first and promote to blocking once the graph drains to
  zero load edges — a red blocking lint must never enter the deploy gate.
...
- **The cross-host graph diff has a cost.** The invariance lint builds each host's graph and diffs the edge
  maps; if the builder is slow to import, the lint is expensive and belongs in a deploy-scope gate rather
  than every commit.

## Known uses

**Designed, not yet landed.** The following are the ratified design's artifacts; none is built or
instantiated at the time of writing (no edge-intent enum, no typed Scheduler, no per-host profile, no lints
exist yet — the build is a later phase). Listed to ground the mechanism, flagged design-stage:

- A `CORRECTNESS`/`COST_GATE`/`LOAD` edge-intent axis on the deploy dependency model, orthogonal to the
  existing artifact-producer edge kind, with load edges migrated out of the graph.
- A typed `Scheduler` mapping a per-host `(concurrency-ceiling, budget)` profile to a concurrency plan plus
  a cost-gate honor set; the stress-test host is a single profile field, so moving the stress test from one
  host to another is a one-cell edit, not a re-architecture.
- Two lints — one asserting no load edge survives in the graph and the edge set is host-identical over the
  roster intersection, one forbidding the retired driver name — both landing audit-only first, then blocking
  once the graph drains to zero load edges.
