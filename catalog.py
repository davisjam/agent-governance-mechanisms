...
LANDING_INTRO = """  <!-- ===================== HERO 2-up =====================
  ...
  <div class="fam">
    ...
    <details class="card" id="card-it-cant-lie">
      ...
      <div class="cd-body">
        ...
        give. But hold the honest other half: a green gate proves the model and the code <b>agree</b>, not that
        either is <i>right</i>. A model derived from the code is a mirror — it records a missing check as
        correctly absent, and the gate agrees with the bug. Only a model <b>authored from intent</b> catches a
        wrong "is."</p>
  ...
  <div class="fam">
    ...
    <details class="card" id="card-generative-validation">
      <summary>
        <span class="cd-kicker">Generate to falsify</span>
        <span class="cd-title">The sharpest sensor generates its own inputs</span>
        <span class="cd-frame">One move, three techniques: hunt a counterexample to a model-supplied spec.</span>
        <span class="cd-toggle" aria-hidden="true"></span>
      </summary>
      <div class="cd-body">
        <p>Most tests reproduce a single dot — one input, one expected output — and say nothing about the input
        next door. There is a sharper sensor: <b>generate inputs and try to falsify a specification the model
        supplies.</b> Property-based testing, fuzzing, and the synthesis of fuzzing with model-based engineering
        are the same move three times, differing only in what the spec <i>is</i> and how the inputs get made. The
        spec is a model; the generator re-hunts a counterexample on every change — the two theses standing in one
        place, a compact model and a machine that never stops trying to break it.</p>
  ...
  <div class="fam">
    <div class="fam-h">Where the engineer's job goes</div>
    <div class="masonry">
    <details class="card wide accent" id="card-engineers-job" open>
      <summary>
        <span class="cd-kicker">The claim the book climbs toward</span>
        <span class="cd-title accent-t">Models are the universal language of engineering</span>
        <span class="cd-frame">The engineer's job relocates from the accidental to the essential.</span>
        <span class="cd-toggle" aria-hidden="true"></span>
      </summary>
      <div class="cd-body">
        <p>Every mature engineering discipline reasons about the thing it builds through a <b>model</b>, not the
        artifact itself — the load diagram, the circuit schematic, the control loop. Software could not, because
        holding a current model honest by hand was too expensive to afford at pace, so the field spent its best
        effort on the <i>accidental</i> complexity — syntax, builds, dependency hell — and starved the
        <i>essential</i>: the system's real properties, its correctness, its design.</p>
        <p>Agents change the arithmetic. They absorb the accidental work — they type the code, keep the map in
        sync, hold the line for cents — so for the first time the essential part can get the attention. The
        engineer's responsibility does not shrink; it <b>relocates upward</b>, from "write correct code" to
        "author the model that says what correct <i>means</i>, and decide what belongs in it." And because the
        map is finally cheap to draw and keep, modeling stops being architect-only work — <b>every</b> engineer
        now works at the level where the properties live. Modeling democratizes. That is the job the engineer's
        work becomes: fluent authorship in the language every engineering discipline has always spoken.</p>
